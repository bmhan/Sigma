...
# Gain indices: 14, 17, .. ,50 + 80, 83, 86, 89
gain_array = range(14, 53, 3) + range(80, 92, 3)

# enter test parameters
board = str(raw_input("Enter board number: "))
...
# bandwidth
bw = 10.0 #float(input("Enter BW in MHz: "))

# open data file
file_name = '..\\data\\RX_LowGainSweep_chip'+part+'_temp'+temperature+'_bw'+str(bw)+'.csv'
...
LO_Freq_MHz = float(input("Enter LO frequency in MHz: "))
Rx_Path = int(input("Enter 1 for RX1 (Primary), 2 for RX2 (Div) and 3 for both: "))
FE_Path  = str(raw_input("Enter front end path - L1 or L2 or H1 or H2: "))
...
# open communication
rlib.OpenDevice()
#dutlib.selectVersion(1.6)
dutlib.selectVersion(1.0)
...
gain_table_name = "..\\tables\\RxGainTable_LowGain_Blocker.csv"
...
for gain_index in gain_array:
	...
	dutlib.loadRxFiltCompVals(gain_index,bw, gain_table_name)
	...
	for bb_kHz in bb_freq_array:
		...
		# setup RF input
		# SG5172.SetSgOn(sg, RF_Freq_MHz*1e6, Pin_dBm) # Set Rx RF in
