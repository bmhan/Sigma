...
class LTE_Test:
    def __init__ (self, master):
        ...
        CABLELabel.grid(row = 3, column = 0)
    ...
    def test(self):
        ...
        with open ("miniUT.setup", 'rb') as file:
            data = json.load(file)
            if self.defaultVal.get() == 0:
                if (self.HOSTText.get()):
                    data["HOST"] = self.HOSTText.get()
                if (self.CABLEText.get()):
                    data["CABLE_LOSS_DB"] = self.CABLEText.get()
                if (self.SNText.get()):
                    data["SN"] = self.SNText.get()
            else:
                    data["HOST"] = data["DEFAULT_HOST"]
                    data["CABLE_LOSS_DB"] = data["DEFAULT_CABLE_LOSS_DB"]
                    data["SN"] = data["DEFAULT_SN"]
            file.close()
        ...
        child = subprocess.Popen(['python','-u','test_miniUT_shortened.py'], stdout = subprocess.PIPE)
